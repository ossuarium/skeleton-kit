= Actions
Actions are modules that add instance methods to +Bit+ objects.
Bits can be grouped together. To create a new group, run

  $ rake mk:group[new_group_name]
...
The +KitActionsDefault+ module is loaded for bits without a group.
